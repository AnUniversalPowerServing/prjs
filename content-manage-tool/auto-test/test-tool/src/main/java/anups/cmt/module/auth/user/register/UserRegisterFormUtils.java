...
public class UserRegisterFormUtils extends AutomationBase implements UserRegisterForm {
	...
	public String checkEmptyGenInfoForm() throws InterruptedException, AWTException {
	   ...
	   if(FORM_GENINFO_ALERTMSG_EMPTYFORM.equalsIgnoreCase(alertResponse)) {
		   isPassed = true;
		   status = Bootstrap.buildLabel("PASSED", "success");
		   
		   // Check Highlights of Error and Success Ticks
	   }
	  sb.append(Bootstrap.buildH5Heading("General Info - Form Submit (Without Filling Form):"));
	  sb.append(Bootstrap.buildTable(Bootstrap.buildTableRow(new String[] {"Error Message",alertResponse+" (Shown and Expected)", status }, isPassed)));
	  return sb.toString();
	}
